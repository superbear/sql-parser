...
LegacySetopsPrecedenceEnabled bool;

// When false, a literal with an exponent would be converted into
// double type rather than decimal type.
LegacyExponentLiteralAsDecimalEnabled bool;

// When true, the behavior of keywords follows ANSI SQL standard.
SQLStandardKeywordBehavior bool;

// When true, double quoted literals are identifiers rather than STRINGs.
DoubleQuotedIdentifiers bool;
...
ParameterSubstitutionEnabled bool;
...
LegacyIdentifierClauseOnly bool;
...
SingleCharacterPipeOperatorEnabled bool;
}

@members {
func (p *SparkSqlParser) isOperatorPipeStart() bool {
    if (!p.SingleCharacterPipeOperatorEnabled) {
		return false;
    }

    la := p.GetTokenStream().LA(2); // Look ahead 2 tokens (current is PIPE, check what follows)
    return la == SparkSqlParserSELECT || la == SparkSqlParserEXTEND || la == SparkSqlParserSET || la == SparkSqlParserDROP ||
		la == SparkSqlParserAS || la == SparkSqlParserWHERE || la == SparkSqlParserPIVOT || la == SparkSqlParserUNPIVOT ||
		la == SparkSqlParserTABLESAMPLE || la == SparkSqlParserINNER || la == SparkSqlParserCROSS || la == SparkSqlParserLEFT ||
		la == SparkSqlParserRIGHT || la == SparkSqlParserFULL || la == SparkSqlParserNATURAL || la == SparkSqlParserSEMI ||
		la == SparkSqlParserANTI || la == SparkSqlParserJOIN || la == SparkSqlParserUNION || la == SparkSqlParserEXCEPT ||
		la == SparkSqlParserSETMINUS || la == SparkSqlParserINTERSECT || la == SparkSqlParserORDER || la == SparkSqlParserCLUSTER ||
		la == SparkSqlParserDISTRIBUTE || la == SparkSqlParserSORT || la == SparkSqlParserLIMIT || la == SparkSqlParserOFFSET ||
		la == SparkSqlParserAGGREGATE || la == SparkSqlParserWINDOW || la == SparkSqlParserLATERAL;
}
}
...
singleStatement
    : (statement|setResetStatement) SEMICOLON*
    ;
...
queryTerm
    ...
    | left=queryTerm {p.LegacySetopsPrecedenceEnabled}?
    ...
    | left=queryTerm {p.isOperatorPipeStart()}? PIPE operatorPipeRightSide                 #operatorPipeStatement
    ;
...
valueExpression
    ...
    | left=valueExpression {!p.isOperatorPipeStart()}? operator=PIPE right=valueExpression     #arithmeticBinary
...
identifier
    : strictIdentifier
    | {!p.SQLStandardKeywordBehavior}? strictNonReserved
    ;
...
strictIdentifier
    ...
    | {!p.LegacyIdentifierClauseOnly}? IDENTIFIER_KW LEFT_PAREN stringLit RIGHT_PAREN  #identifierLiteral
    | {p.SQLStandardKeywordBehavior}? ansiNonReserved #unquotedIdentifier
    | {!p.SQLStandardKeywordBehavior}? nonReserved    #unquotedIdentifier
    ;
...
simpleStrictIdentifier
    ...
    | {p.SQLStandardKeywordBehavior}? ansiNonReserved #simpleUnquotedIdentifier
    | {!p.SQLStandardKeywordBehavior}? nonReserved    #simpleUnquotedIdentifier
    ;

quotedIdentifier
    : BACKQUOTED_IDENTIFIER
    | {p.DoubleQuotedIdentifiers}? DOUBLEQUOTED_STRING
    ;
...
number
    : {!p.LegacyExponentLiteralAsDecimalEnabled}? MINUS? EXPONENT_VALUE #exponentLiteral
    | {!p.LegacyExponentLiteralAsDecimalEnabled}? MINUS? DECIMAL_VALUE  #decimalLiteral
    | {p.LegacyExponentLiteralAsDecimalEnabled}? MINUS? (EXPONENT_VALUE | DECIMAL_VALUE) #legacyDecimalLiteral
...
singleStringLitWithoutMarker
    ...
    | {!p.DoubleQuotedIdentifiers}? DOUBLEQUOTED_STRING                                        #singleDoubleQuotedStringLiteralValue
    ;
...
parameterMarker
    : {p.ParameterSubstitutionEnabled}? namedParameterMarker                                   #namedParameterMarkerRule
    | {p.ParameterSubstitutionEnabled}? QUESTION                                               #positionalParameterMarkerRule
    ;
...
// When `p.SQLStandardKeywordBehavior=true`, there are 2 kinds of keywords in Spark SQL.
// - Reserved keywords:
...
// When `p.SQLStandardKeywordBehavior=false`, there are 2 kinds of keywords in Spark SQL.
// - Non-reserved keywords:
//     Same definition as the one when `p.SQLStandardKeywordBehavior=true`.
